...
/-- Concrete project-length threshold certificate calibrated to the singleton
tail-gap rejection extractor.  This avoids the arbitrary executable-search
calibration parameter: the singleton extractor generated from the same
tail-gap certificate has witness definitionally `max upperN threshold`, and
the no-fallback project-length endpoint has `upperN = 0`. -/
theorem projectLengthThresholdCertificate_eq_singletonTailGapWitnessOfTimeBound
    {scale_data : InternalPudlakTheorem5ScaleData}
    ...
    let singleton :=
      singletonGapRejectionInputOfTailGap
        lengthCodeAt
        (powerBoundRawCode_injective_of_scale_strict
          (your_scale_strict_theorem time_bound_strict exponent_ne_zero))
        tail_gap
  ...
  have hbig_singleton :
    ...
    have hwitness :=
      singletonGapRejectionInputOfTailGap_witness_eq_max
        ...
        (powerBoundRawCode_injective_of_scale_strict
          (your_scale_strict_theorem time_bound_strict exponent_ne_zero))
        tail_gap upper.U upper.polynomial upper.upperN
    rw [hwitness]
    simp [bigN, hupperN]
  exact
    ⟨hbig_singleton,
...
/-- Closed clean project-length endpoint calibrated to the singleton tail-gap
extractor.  This removes the executable-search calibration hypothesis from the
public project-length non-rationality route. -/
theorem projectLengthThreshold_not_rational_eq_singletonTailGapWitnessOfTimeBound
    {scale_data : InternalPudlakTheorem5ScaleData}
    {L : _root_.FirstOrder.Language.{u, v}} {α : Type w} {n : Nat}
    {Ax : L.BoundedFormula α n → Prop}
    {A B : Nat → L.BoundedFormula α n}
    ...
    ¬ _root_.is_rational _root_.euler_mascheroni :=
  fun hrat =>
    (projectLengthThresholdCertificate_eq_singletonTailGapWitnessOfTimeBound
      left_family right_family lengthCodeAt time_bound_strict
      exponent_ne_zero tail_gap lengthCodeAt_eq_conj_source
...
/-- Closed non-rationality endpoint for the concrete project-length threshold
certificate calibrated to an executable search witness.  This is the clean
project-length route: the computed threshold is identified with
`search.witness`, and the final contradiction does not pass through root
`proof_length` or payload predicates. -/
theorem projectLengthThreshold_not_rational_eq_executableSearchWitnessOfTimeBound
    {scale_data : InternalPudlakTheorem5ScaleData}
    {L : _root_.FirstOrder.Language.{u, v}} {α : Type w} {n : Nat}
    {Ax : L.BoundedFormula α n → Prop}
    {A B : Nat → L.BoundedFormula α n}
    ...
    ¬ _root_.is_rational _root_.euler_mascheroni :=
  fun hrat =>
    (projectLengthThresholdCertificate_eq_executableSearchWitnessOfTimeBound
      left_family right_family lengthCodeAt time_bound_strict
      exponent_ne_zero tail_gap enumeration search witness_calibration
...
actual proof-length gap.  The generated search gap is not arbitrary: its
witness is definitionally `max N threshold`, so the final endpoint can expose a
computable large-`N` expression instead of only an opaque search witness. -/
def finalScaleSizeTailGapExactProofGapCheckerCoreInput
    {scale_data : InternalPudlakTheorem5ScaleData}
    (timeConstructiblePower_strict :
      ∀ {a b : Nat}, a < b →
        scale_data.time_constructible_bound a ^ scale_data.exponent <
    ...
    ConcretePAHilbertPowerBoundFinalScaleSizeExactProofGapCheckerCoreInput
      scale_data where
  timeConstructiblePower_strict := timeConstructiblePower_strict
  proof_length_gap := proof_length_tail_gap.toComputableSearchGapCertificate
  proof_length_eq_scale := proof_length_eq_scale

/-- Final exact checker core produced by
`finalScaleSizeTailGapExactProofGapCheckerCoreInput`. -/
def finalScaleSizeTailGapExactProofGapCore
    {scale_data : InternalPudlakTheorem5ScaleData}
    (timeConstructiblePower_strict :
      ∀ {a b : Nat}, a < b →
        scale_data.time_constructible_bound a ^ scale_data.exponent <
  ...
  (finalScaleSizeTailGapExactProofGapCheckerCoreInput
    ...
    proof_length_eq_scale).toFinalExactCheckerCoreInput

theorem finalScaleSizeTailGapExactProofGap_witness_eq_max
    {scale_data : InternalPudlakTheorem5ScaleData}
    (timeConstructiblePower_strict :
      ∀ {a b : Nat}, a < b →
        scale_data.time_constructible_bound a ^ scale_data.exponent <
    ...
    ((finalScaleSizeTailGapExactProofGapCore
        timeConstructiblePower_strict proof_length_tail_gap
        proof_length_eq_scale).proof_length_gap
      |>.gap_for_polynomial_upper U hU).witness N =
      max N (proof_length_tail_gap.gap_for_polynomial_upper U hU).threshold := by
  rfl

theorem finalScaleSizeTailGapExactProofGapEndpointCLine_computed_n_eq_max
    {scale_data : InternalPudlakTheorem5ScaleData}
    (timeConstructiblePower_strict :
      ∀ {a b : Nat}, a < b →
        scale_data.time_constructible_bound a ^ scale_data.exponent <
    ...
    (finalExactEndpointOfConstantProjectionCLine
      core constant_projection hkernel hchecker hlength).computedCollisionNOfRationality hrat =
      max upper.upperN
      ...
      core constant_projection hkernel hchecker hlength).computedCollisionNOfRationality hrat =
        (core.proof_length_gap.gap_for_polynomial_upper
          ...
          simpa [core, upper] using
            finalExactEndpointOfConstantProjectionCLine_computed_n_eq
              core constant_projection hkernel hchecker hlength hrat
    _ =
        max upper.upperN
          (proof_length_tail_gap.gap_for_polynomial_upper
            upper.U upper.polynomial).threshold := by
...
/-- C-line closure for the tail-gap generated final exact checker core.  This
is the same corrected actual endpoint as
`finalExactEndpointOfConstantProjectionCLine_closure`, but its computed witness
is pinned to the explicit tail-gap number
`max upperN (tail_gap ...).threshold`. -/
theorem finalScaleSizeTailGapExactProofGapEndpointCLine_closure
    {scale_data : InternalPudlakTheorem5ScaleData}
    (timeConstructiblePower_strict :
      ∀ {a b : Nat}, a < b →
        scale_data.time_constructible_bound a ^ scale_data.exponent <
    ...
    let endpoint :=
      finalExactEndpointOfConstantProjectionCLine
        core constant_projection hkernel hchecker hlength;
    endpoint.Audit ∧
      Nonempty
        (ComputableSearchGapCertificate
          (actualProofLengthMeasured
  ...
  let endpoint :=
    finalExactEndpointOfConstantProjectionCLine
      core constant_projection hkernel hchecker hlength
  have hclosure :=
    finalExactEndpointOfConstantProjectionCLine_closure
      core constant_projection hkernel hchecker hlength
  exact
    ⟨hclosure.1,
      hclosure.2.1,
