...
/-!
# Public bounds for one-formula proof-root failure

After a successful sequent parse, tags zero, three, four, five, six, and nine
can fail on their first formula parse.  This module installs the successful
sequent trace and the complete no-output formula trace in one canonical table,
then bounds every exposed endpoint coordinate by the original root-input
weight.
-/
...
namespace FoundationCompactNumericListedDirectProofRootOneFormulaFailurePublicBounds
...
open FoundationCompactNumericListedDirectProofRootOneFormulaPublicBounds
...
private theorem compactFormulaParserNoOutputLocalTrace_weight_le_rootBound
  ...
  have hrows := compactAdditiveValueWeight_list_le states
    (compactNumericFormulaParserStateWeightBound
      ...
      (compactSyntaxRunFuelBound tokens))
    (fun state hstate => by
      exact compactFormulaParserLocalTrace_member_state_weight_le
        hlocal hstate)
  have hraw :
      compactAdditiveValueWeight states <=
...
def compactProofRootOneFormulaFailureParserCoordinateSizeBound
    (inputWeight : Nat) : Nat :=
  let sharedBound :=
...
def compactProofRootOneFormulaFailureLocalCoordinateSizeBound
    (inputWeight : Nat) : Nat :=
  let sharedBound :=
    compactProofRootOneFormulaSharedCoordinateSizeBound inputWeight
  sharedBound +
    ...
    compactProofRootOneFormulaFailureParserCoordinateSizeBound inputWeight +
    inputWeight + 12

def compactProofRootOneFormulaFailurePublicCoordinateSizeBound
    (inputWeight : Nat) : Nat :=
  compactProofRootOneFormulaFailureLocalCoordinateSizeBound inputWeight + 1

structure CompactProofRootOneFormulaFailurePublicCoordinateBounds
    (tokenTable width tokenCount inputStart inputFinish
...
theorem
    exists_compactProofRootOneFormulaFailureEndpointBoundedGraph_of_results_with_publicBounds
    (tag binderArity : Nat) (body : List Nat)
    (values : List (List Nat)) (afterSequent : List Nat)
    ...
    ∃ bodyStart bodyFinish endpointBound,
      CompactAdditiveNatListDirectLayout
        ...
        CompactProofRootOneFormulaFailureEndpointBoundedGraph
          tokenTable width tokenCount inputStart inputFinish
            bodyStart bodyFinish endpointBound ∧
        CompactProofRootOneFormulaFailurePublicCoordinateBounds
          tokenTable width tokenCount inputStart inputFinish
            bodyStart bodyFinish endpointBound
            (compactProofRootOneFormulaFailurePublicCoordinateSizeBound
              inputWeight) := by
  let input := tag :: body
  ...
  have hrootTag : root.1 <= 10 := by
    dsimp only [root]
    rcases htagBinder with hzero | hone
    · rcases hzero with ⟨htags, _hbinder⟩
      rcases htags with rfl | rfl | rfl | rfl <;> omega
    · rcases hone with ⟨htags, _hbinder⟩
      rcases htags with rfl | rfl <;> omega
  ...
  have houtput : compactSyntaxParserStateOutput
      ((compactSyntaxParserStep^[compactSyntaxRunFuelBound afterSequent])
        (compactFormulaParserInitialState binderArity afterSequent)) =
      none := by
    simpa [compactFormulaTokenParser,
      compactFormulaTokenParserRun] using hformula
  rcases
    ...
    have hraw :=
      CompactAdditiveStructuredListElementRowLayouts.natConsRows
        hwitnessBodyRows hwitnessInputRows
          (show input =
            tag :: (compactSequentListTokens Gamma ++ afterSequent) by
              rfl)
  ...
  have hfailureBounded :
      CompactParserSyntaxNoOutputExactEndpointBoundedGraph
        tokenTable width tokenCount finalStart finalFinish
          1 binderArity 0 (2 ^ failureActualBound) := by
  ...
  have hgraph :
    ...
    unfold CompactProofRootOneFormulaFailureEndpointGraph
    dsimp only [coordinates]
    exact
      ⟨hinputWitness, hsequentGraph.1, hcoordinateTagBinder,
        hconsCoordinateTag, hsequentBounded, hfailureBounded⟩
  ...
  let failureCoordinateBound :=
    compactProofRootOneFormulaFailureParserCoordinateSizeBound
      inputWeight
  have hfailureCoordinateBound :
    ...
    exact
      compactSequentFormulaStepPublicCoordinateSizeBound_mono
        hwidthValue htokenCountValue
  let localBound :=
    compactProofRootOneFormulaFailureLocalCoordinateSizeBound
      inputWeight
  ...
  have hbounded :
    ...
    refine
      ⟨inputBoundary,
        ...
        binderArity,
          nat_le_two_pow_of_size_le hbinderAritySize,
...
end FoundationCompactNumericListedDirectProofRootOneFormulaFailurePublicBounds
