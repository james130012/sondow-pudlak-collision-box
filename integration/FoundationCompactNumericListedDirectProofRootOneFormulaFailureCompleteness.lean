import integration.FoundationCompactNumericListedDirectProofRootOneFormulaFailureBoundedFormula
...
/-!
# Completeness of one-formula proof-root failure

A successful sequent trace and the following failed formula trace are laid out
with the outer proof-root input in one canonical table.  An auxiliary root row
stores the parsed sequent values only; no theorem assumes that this row is a
successful public root-parser output.
-/
...
namespace FoundationCompactNumericListedDirectProofRootOneFormulaFailureCompleteness
...
theorem exists_compactProofRootOneFormulaFailureEndpointGraph_of_results_with_inputLayout
  ...
  let coordinates : CompactProofRootOneFormulaFailureEndpointCoordinates :=
  ...
  exact ⟨hinputWitness, hsequentGraph.1, htagBinder, hcons,
    hsequentBounded, hfailureBounded⟩

theorem exists_compactProofRootOneFormulaFailureEndpointGraph_of_results
    (tag binderArity : Nat) (body : List Nat) (values : List (List Nat))
    (afterSequent : List Nat)
    (htagBinder : CompactProofRootFormulaFailureTagBinder tag binderArity)
    (hsequent : compactSequentTokenValueParser body =
    ...
    ∃ coordinates : CompactProofRootOneFormulaFailureEndpointCoordinates,
      CompactProofRootOneFormulaFailureEndpointGraph
        tokenTable width tokenCount inputStart inputFinish
          bodyStart bodyFinish coordinates := by
  rcases
      exists_compactProofRootOneFormulaFailureEndpointGraph_of_results_with_inputLayout
...
theorem exists_compactProofRootOneFormulaFailureEndpointBoundedGraph_of_results
    (tag binderArity : Nat) (body : List Nat) (values : List (List Nat))
    (afterSequent : List Nat)
    (htagBinder : CompactProofRootFormulaFailureTagBinder tag binderArity)
    (hsequent : compactSequentTokenValueParser body =
  ...
  rcases exists_compactProofRootOneFormulaFailureEndpointGraph_of_results
      tag binderArity body values afterSequent
        htagBinder hsequent hformula with
    ⟨tokenTable, width, tokenCount, inputStart, inputFinish,
      bodyStart, bodyFinish, coordinates, hgraph⟩
...
theorem exists_compactProofRootOneFormulaFailureEndpointBoundedGraph_of_results_with_inputLayout
    (tag binderArity : Nat) (body : List Nat) (values : List (List Nat))
    (afterSequent : List Nat)
    (htagBinder : CompactProofRootFormulaFailureTagBinder tag binderArity)
    (hsequent : compactSequentTokenValueParser body =
...
#print axioms exists_compactProofRootOneFormulaFailureEndpointGraph_of_results
#print axioms exists_compactProofRootOneFormulaFailureEndpointBoundedGraph_of_results

end FoundationCompactNumericListedDirectProofRootOneFormulaFailureCompleteness
