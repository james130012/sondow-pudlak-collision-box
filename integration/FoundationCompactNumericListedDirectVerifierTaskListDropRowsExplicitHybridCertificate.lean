...
/-!
# Explicit hybrid certificate for verifier-task-list prefix dropping
...
namespace FoundationCompactNumericListedDirectVerifierTaskListDropRowsExplicitHybridCertificate
...
private def compactNumericVerifierTaskListDropRowsBody
...
private def compactNumericVerifierTaskListDropRowsRowAtValuationFormula
    (tokenTable width tokenCount sourceBoundary targetBoundary
...
private theorem compactNumericVerifierTaskListDropRowsBody_free_alignment
    (tokenTable width tokenCount sourceBoundary targetBoundary
      valueBound consumed : Nat) :
    Rewriting.free
        (compactNumericVerifierTaskListDropRowsBody
          tokenTable width tokenCount sourceBoundary targetBoundary
            valueBound consumed) =
      compactNumericVerifierTaskListDropRowsRowAtValuationFormula
        tokenTable width tokenCount sourceBoundary targetBoundary
          valueBound consumed := by
  unfold compactNumericVerifierTaskListDropRowsBody
  rw [rewriting_embeddedFormulaSubstitution]
  unfold compactNumericVerifierTaskListDropRowsRowAtValuationFormula
...
private noncomputable def compactNumericVerifierTaskListDropRowsBranchCertificate
    (tokenTable width tokenCount sourceBoundary targetBoundary
    ...
    CheckedHybridValuationBoundedFormulaCertificate
      ...
      (Rewriting.free
        (compactNumericVerifierTaskListDropRowsBody
          tokenTable width tokenCount sourceBoundary targetBoundary
            valueBound consumed)) := by
  rw [compactNumericVerifierTaskListDropRowsBody_free_alignment]
  exact
...
def compactNumericVerifierTaskListDropRowsClosedFormula
    (tokenTable width tokenCount sourceBoundary sourceCount
      targetBoundary targetCount tableWidth valueBound consumed : Nat) :
    ValuationFormula :=
  (Rewriting.emb (ξ := Nat) compactNumericVerifierTaskListDropRowsDef.val) ⇜
...
theorem compactNumericVerifierTaskListDropRowsClosedFormula_alignment
    (tokenTable width tokenCount sourceBoundary sourceCount
      targetBoundary targetCount tableWidth valueBound consumed : Nat) :
    compactNumericVerifierTaskListDropRowsClosedFormula
        tokenTable width tokenCount sourceBoundary sourceCount
  ...
  unfold compactNumericVerifierTaskListDropRowsClosedFormula
  unfold compactNumericVerifierTaskListDropRowsDef
  simp [compactNumericVerifierTaskListDropRowsBody,
    rewriting_ballLT,
...
noncomputable def compactNumericVerifierTaskListDropRowsExplicitHybridCertificate
    (tokenTable width tokenCount sourceBoundary sourceCount
      targetBoundary targetCount tableWidth valueBound consumed : Nat)
    (hgraph : CompactNumericVerifierTaskListDropRows
      tokenTable width tokenCount sourceBoundary sourceCount
        targetBoundary targetCount tableWidth valueBound consumed) :
    HybridCertificate
      (compactNumericVerifierTaskListDropRowsClosedFormula
  ...
  rw [compactNumericVerifierTaskListDropRowsClosedFormula_alignment]
  ...
  let countEq := closedAddEqCertificate sourceCount consumed targetCount
    hgraph.2.1
  let body := compactNumericVerifierTaskListDropRowsBody
    tokenTable width tokenCount sourceBoundary targetBoundary
      valueBound consumed
  let branches := buildExplicitHybridUniversalBranches targetCount
    (fun index hindex =>
      compactNumericVerifierTaskListDropRowsBranchCertificate
        tokenTable width tokenCount sourceBoundary targetBoundary
...
private def compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
...
private def compactNumericVerifierTaskListDropRowsRowAtValuationTermFormula
    (tokenTable width tokenCount sourceBoundary targetBoundary valueBound : Nat)
...
private theorem
    compactNumericVerifierTaskListDropRowsBodyAtValuationTerm_free_alignment
    (tokenTable width tokenCount sourceBoundary targetBoundary valueBound : Nat)
    (consumedTerm : ValuationTerm) :
    Rewriting.free
        (compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
          tokenTable width tokenCount sourceBoundary targetBoundary valueBound
            consumedTerm) =
      compactNumericVerifierTaskListDropRowsRowAtValuationTermFormula
        tokenTable width tokenCount sourceBoundary targetBoundary valueBound
          consumedTerm := by
  unfold compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
  rw [rewriting_embeddedFormulaSubstitution]
  unfold compactNumericVerifierTaskListDropRowsRowAtValuationTermFormula
  unfold compactNumericVerifierTaskBoundedRowsEqAtValuationFormula
  congr 1
  funext coordinate
...
private noncomputable def
    compactNumericVerifierTaskListDropRowsBranchAtValuationTermCertificate
    (tokenTable width tokenCount sourceBoundary targetBoundary valueBound
      consumed index : Nat)
    (consumedTerm : ValuationTerm)
    ...
    CheckedHybridValuationBoundedFormulaCertificate
      ...
      (Rewriting.free
        (compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
          tokenTable width tokenCount sourceBoundary targetBoundary valueBound
            consumedTerm)) := by
  rw [compactNumericVerifierTaskListDropRowsBodyAtValuationTerm_free_alignment]
  exact
    compactNumericVerifierTaskBoundedRowsEqAtValuationExplicitHybridCertificate
      (extendValuation index zeroValuation)
      tokenTable width tokenCount sourceBoundary targetBoundary valueBound
...
def compactNumericVerifierTaskListDropRowsAtValuationFormula
    (tokenTable width tokenCount sourceBoundary sourceCount
...
theorem compactNumericVerifierTaskListDropRowsAtValuationFormula_alignment
    (tokenTable width tokenCount sourceBoundary sourceCount
      targetBoundary targetCount tableWidth valueBound : Nat)
    (consumedTerm : ValuationTerm) :
      ...
      (“!!consumedTerm ≤ !!(shortBinaryNumeralTerm sourceCount)” ⋏
        (“!!(shortBinaryNumeralTerm sourceCount) =
          ...
          (compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
            ...
            (shortBinaryNumeralTerm targetCount))) := by
  unfold compactNumericVerifierTaskListDropRowsAtValuationFormula
  unfold compactNumericVerifierTaskListDropRowsDef
  simp [compactNumericVerifierTaskListDropRowsBodyAtValuationTerm,
...
noncomputable def
  ...
  rw [compactNumericVerifierTaskListDropRowsAtValuationFormula_alignment]
  ...
  let body := compactNumericVerifierTaskListDropRowsBodyAtValuationTerm
    tokenTable width tokenCount sourceBoundary targetBoundary valueBound
      consumedTerm
  let branches := buildExplicitHybridUniversalBranches targetCount
    (fun index hindex =>
...
#print axioms compactNumericVerifierTaskListDropRowsClosedFormula
#print axioms compactNumericVerifierTaskListDropRowsClosedFormula_alignment
#print axioms compactNumericVerifierTaskListDropRowsExplicitHybridCertificate
#print axioms compactNumericVerifierTaskListDropRowsAtValuationFormula
#print axioms compactNumericVerifierTaskListDropRowsAtValuationFormula_alignment
#print axioms compactNumericVerifierTaskListDropRowsAtValuationExplicitHybridCertificate

end FoundationCompactNumericListedDirectVerifierTaskListDropRowsExplicitHybridCertificate
