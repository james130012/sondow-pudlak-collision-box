...
/-!
# Completeness of trailing-term proof-root failure

Tag six has a successful sequent trace and binder-one formula trace followed by
a failed closed-term trace.  All three traces are installed in one canonical
token table.  The auxiliary root row stores only the parsed sequent values; no theorem
...
namespace FoundationCompactNumericListedDirectProofRootFormulaTermFailureCompleteness
...
theorem exists_compactProofRootFormulaTermFailureEndpointGraph_of_results_with_inputLayout
    ...
    (hfirst : compactFormulaTokenParser 1 afterSequent = some secondInput)
    (hsecond : compactTermTokenParser 0 secondInput = none) :
  ...
  rcases
      (compactFormulaTokenParser_eq_some_iff_exists_directTrace
        1 afterSequent secondInput).mp hfirst with
  ...
  have hsecondOutput : compactSyntaxParserStateOutput
      ((compactSyntaxParserStep^[compactSyntaxRunFuelBound secondInput])
        (compactTermParserInitialState 0 secondInput)) = none := by
    simpa [compactTermTokenParser, compactTermTokenParserRun] using
      hsecond
  rcases (compactParserOutput_eq_iff_exists_localTrace
      compactSyntaxParserStep (compactSyntaxRunFuelBound secondInput)
      (compactTermParserInitialState 0 secondInput) none).mp
        hsecondOutput with
  ...
  have hfirstLocal : CompactParserOutputLocalTraceValid
      ...
      (afterSequent, [(1, 1, 0)], none) (some secondInput) firstStates := by
  ...
  have hsecondLocal : CompactParserOutputLocalTraceValid
      compactSyntaxParserStep (compactSyntaxRunFuelBound secondInput)
      (secondInput, [(0, 0, 0)], none) none secondStates := by
    simpa [compactTermParserInitialState, compactTermTask] using
      hsecondValid
  ...
  rcases exists_compactParserSyntaxNoOutputExactEndpointGraph_of_rows
      hsecondInputLayout hsecondStateRows htermStartWell hsecondLocal with
  ...
  exact ⟨hinputWitness, hsequentGraph.1, rfl, hcons,
    hsequentBounded, hfirstBounded, hfailureBounded⟩

theorem exists_compactProofRootFormulaTermFailureEndpointGraph_of_results
    (body : List Nat) (values : List (List Nat))
    (afterSequent secondInput : List Nat)
    (hsequent : compactSequentTokenValueParser body =
      some (values, afterSequent))
    ...
    ∃ coordinates : CompactProofRootFormulaTermFailureEndpointCoordinates,
      CompactProofRootFormulaTermFailureEndpointGraph
        tokenTable width tokenCount inputStart inputFinish
          bodyStart bodyFinish coordinates := by
  rcases
      exists_compactProofRootFormulaTermFailureEndpointGraph_of_results_with_inputLayout
        body values afterSequent secondInput hsequent hfirst hsecond with
    ⟨tokenTable, width, tokenCount, inputStart, inputFinish,
...
theorem exists_compactProofRootFormulaTermFailureEndpointBoundedGraph_of_results
    (body : List Nat) (values : List (List Nat))
    (afterSequent secondInput : List Nat)
    (hsequent : compactSequentTokenValueParser body =
      some (values, afterSequent))
  ...
  rcases exists_compactProofRootFormulaTermFailureEndpointGraph_of_results
      body values afterSequent secondInput hsequent hfirst hsecond with
    ⟨tokenTable, width, tokenCount, inputStart, inputFinish,
      bodyStart, bodyFinish, coordinates, hgraph⟩
...
theorem exists_compactProofRootFormulaTermFailureEndpointBoundedGraph_of_results_with_inputLayout
    (body : List Nat) (values : List (List Nat))
    (afterSequent secondInput : List Nat)
    (hsequent : compactSequentTokenValueParser body =
      some (values, afterSequent))
...
#print axioms exists_compactProofRootFormulaTermFailureEndpointGraph_of_results
#print axioms exists_compactProofRootFormulaTermFailureEndpointBoundedGraph_of_results

end FoundationCompactNumericListedDirectProofRootFormulaTermFailureCompleteness
