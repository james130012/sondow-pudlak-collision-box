...
/-!
# Explicit hybrid certificate for additive triple boundary rows

Each universal row branch installs its two bounded cursors, their fixed-width
boundary-table entries, and the three-token span equation directly from the semantic
row graph.
-/
...
namespace FoundationCompactNumericListedDirectAdditiveTripleBoundaryRowsExplicitHybridCertificate
...
def compactAdditiveTripleBoundaryRowsClosedFormula
    (tokenCount count boundaryTable : Nat) : ValuationFormula :=
  (Rewriting.emb (ξ := Nat) compactAdditiveTripleBoundaryRowsDef.val) ⇜
...
def compactAdditiveTripleBoundaryRowsTerminal
...
def compactAdditiveTripleBoundaryRowsBody
    (tokenCount boundaryTable : Nat) : ArithmeticSemiformula Nat 1 :=
  ((compactAdditiveTripleBoundaryRowsTerminal tokenCount boundaryTable).bexsLTSucc
      (closedShift 2 (shortBinaryNumeralTerm tokenCount))).bexsLTSucc
    (closedShift 1 (shortBinaryNumeralTerm tokenCount))

/-- The explicit bounded-universal presentation of the triple-boundary rows. -/
def compactAdditiveTripleBoundaryRowsExplicitFormula
    (tokenCount count boundaryTable : Nat) : ValuationFormula :=
  (compactAdditiveTripleBoundaryRowsBody tokenCount boundaryTable).ballLT
    (shortBinaryNumeralTerm count)

/-- Exact syntax alignment with `compactAdditiveTripleBoundaryRowsDef`. -/
theorem compactAdditiveTripleBoundaryRowsClosedFormula_alignment
  ...
  unfold compactAdditiveTripleBoundaryRowsExplicitFormula
  unfold compactAdditiveTripleBoundaryRowsBody
  unfold compactAdditiveTripleBoundaryRowsTerminal
  unfold compactAdditiveTripleBoundaryRowsDef
...
/-- Concrete witnesses installed by one triple-boundary row branch. -/
structure CompactAdditiveTripleBoundaryRowData
...
def compactAdditiveTripleBoundaryRowsBranchTerminal
...
private theorem compactAdditiveTripleBoundaryRowsTerminal_free_alignment
  ...
  unfold compactAdditiveTripleBoundaryRowsTerminal
  unfold compactAdditiveTripleBoundaryRowsBranchTerminal
...
theorem compactAdditiveTripleBoundaryRowsBody_free_alignment
    (tokenCount boundaryTable : Nat) :
    Rewriting.free
  ...
  simp [Rew.q_free, closedShift,
    compactAdditiveTripleBoundaryRowsTerminal_free_alignment]

/-- Substituting the two cursor witnesses exposes all three terminal facts. -/
theorem compactAdditiveTripleBoundaryRowsBranchTerminal_substitution_alignment
    ...
    (compactAdditiveTripleBoundaryRowsBranchTerminal tokenCount boundaryTable) ⇜
      ...
      (compactFixedWidthEntryAtValuationFormula
        ...
        (compactFixedWidthEntryAtValuationFormula
          ...
          “!!(shortBinaryNumeralTerm right) =
            !!(shortBinaryNumeralTerm left) + 3”)) := by
  unfold compactAdditiveTripleBoundaryRowsBranchTerminal
...
private theorem termValue_arithmeticThree (valuation : Nat -> Nat) :
    termValue valuation (‘3’ : ValuationTerm) = 3 := by
  simp [termValue, LO.FirstOrder.Semiterm.val_operator]

noncomputable def tripleEqualityCertificate
  ...
  let direct := CheckedHybridValuationBoundedFormulaCertificate.positiveAtomic
    valuation Language.Eq.eq ![shortBinaryNumeralTerm right, rightTerm] (by
      ...
      simp only [rightTerm, termValue_shortBinaryNumeralTerm,
        termValue_arithmeticAdd, termValue_arithmeticThree]
      exact htriple)
  exact .cast (LO.FirstOrder.Semiformula.Operator.eq_def _ _).symm direct

noncomputable def compactAdditiveTripleBoundaryRowsBranchCertificate
    ...
    HybridCertificate (extendValuation index zeroValuation)
      (Rewriting.free
        (compactAdditiveTripleBoundaryRowsBody tokenCount boundaryTable)) := by
  ...
  let terminalParts :=
    CheckedHybridValuationBoundedFormulaCertificate.conjunction
      ...
      (CheckedHybridValuationBoundedFormulaCertificate.conjunction
        ...
        (tripleEqualityCertificate valuation data.left data.right
          data.triple))
...
noncomputable def compactAdditiveTripleBoundaryRowsDirectHybridBranchesAtCount
    (tokenCount count boundaryTable : Nat)
    ...
    CheckedHybridValuationUniversalBranches zeroValuation
      (compactAdditiveTripleBoundaryRowsBody tokenCount boundaryTable) count :=
...
/-- The evaluated short binary numeral agrees with the public row count. -/
theorem compactAdditiveTripleBoundaryRowsShortNumeralBound_eq_count
    (count : Nat) :
    termValue zeroValuation (shortBinaryNumeralTerm count) = count := by
  exact termValue_shortBinaryNumeralTerm zeroValuation count

/-- The same branches at the evaluated bound expected by the universal
certificate constructor. -/
noncomputable def compactAdditiveTripleBoundaryRowsDirectHybridBranches
    (tokenCount count boundaryTable : Nat)
    (rows : (index : Fin count) ->
      CompactAdditiveTripleBoundaryRowData tokenCount boundaryTable index) :
    CheckedHybridValuationUniversalBranches zeroValuation
...
noncomputable def compactAdditiveTripleBoundaryRowsUniversalCertificate
    (tokenCount count boundaryTable : Nat)
    (rows : (index : Fin count) ->
      CompactAdditiveTripleBoundaryRowData tokenCount boundaryTable index) :
...
/-- Canonical row data extracted from the semantic triple-boundary graph. -/
noncomputable def compactAdditiveTripleBoundaryRowDataOfGraph
    (tokenCount count boundaryTable : Nat)
    (hrows : CompactAdditiveTripleBoundaryRows
      tokenCount count boundaryTable)
    (index : Fin count) :
    CompactAdditiveTripleBoundaryRowData tokenCount boundaryTable index := by
...
/-- Build every row witness directly from the semantic triple-boundary graph. -/
noncomputable def compactAdditiveTripleBoundaryRowsExplicitHybridCertificateOfGraph
    (tokenCount count boundaryTable : Nat)
    (hrows : CompactAdditiveTripleBoundaryRows
      tokenCount count boundaryTable) :
    HybridCertificate zeroValuation
...
#print axioms compactAdditiveTripleBoundaryRowsClosedFormula
#print axioms compactAdditiveTripleBoundaryRowsClosedFormula_alignment
#print axioms compactAdditiveTripleBoundaryRowsFromRowDataExplicitHybridCertificate
#print axioms compactAdditiveTripleBoundaryRowsExplicitHybridCertificateOfGraph

end FoundationCompactNumericListedDirectAdditiveTripleBoundaryRowsExplicitHybridCertificate
