...
/-!
# Proof-producing compiler for true Sigma-one formulas under a valuation

Unbounded existential witnesses are extracted from standard-model truth.
Every bounded matrix is delegated to the checked Sigma-zero compiler.  The
certificate stores witnesses and branch choices, but no PA proof, proof code,
proof-existence statement, or proof-length premise.
-/
...
namespace FoundationCompactPAValuationSigmaOneFormulaCompiler
...
abbrev SigmaOneCertificate :=
  CheckedValuationBoundedFormulaCertificate

theorem certificate_nonempty_of_sigmaOne_truth
    (formula : ValuationFormula)
    (hhierarchy : Hierarchy Polarity.sigma 1 formula)
    (valuation : Nat -> Nat)
    (htruth : formulaValue valuation formula) :
    Nonempty (SigmaOneCertificate valuation formula) := by
  ...
  | hand left right =>
      ...
      have hvalues : formulaValue valuation left ∧
        ...
        simpa [formulaValue] using htruth
      rcases certificate_nonempty_of_sigmaOne_truth
  ...
  | hall quantifiedBody =>
      ...
      | ball boundPositive bodyHierarchy =>
          ...
          have hbranchCertificates : ∀ index, index < bound ->
              Nonempty (SigmaOneCertificate
                (extendValuation index valuation)
                (Rewriting.free boundedBody)) := by
            intro index hindex
            exact certificate_nonempty_of_sigmaOne_truth
  ...
  | hexs quantifiedBody =>
      ...
      | bexs boundPositive bodyHierarchy =>
          ...
          simpa [boundedMatrix] using
            (show Nonempty (SigmaOneCertificate valuation
                (∃⁰ boundedMatrix)) from
...
noncomputable def ofSigmaOneTruth
    (formula : ValuationFormula)
    (hhierarchy : Hierarchy Polarity.sigma 1 formula)
    (valuation : Nat -> Nat)
    (htruth : formulaValue valuation formula) :
    SigmaOneCertificate valuation formula :=
  Classical.choice
    (certificate_nonempty_of_sigmaOne_truth
      formula hhierarchy valuation htruth)

noncomputable def compileSigmaOneTruth
    (formula : ValuationFormula)
    (hhierarchy : Hierarchy Polarity.sigma 1 formula)
    (valuation : Nat -> Nat)
    (htruth : formulaValue valuation formula) :
  ...
  (ofSigmaOneTruth formula hhierarchy valuation htruth).compile

#print axioms certificate_nonempty_of_sigmaOne_truth
#print axioms compileSigmaOneTruth

end FoundationCompactPAValuationSigmaOneFormulaCompiler
