...
import integration.FoundationCompactNumericListedDirectParserClosedSuccessAdjacentStepBoundedInstallation

/-!
# One bounded formula for a successful closed-formula parser trace

The endpoint predicate and every adjacent successful closed-parser step share one
...
namespace FoundationCompactNumericListedDirectParserClosedSuccessTraceFormula
...
open FoundationCompactNumericListedDirectParserClosedSuccessAdjacentStepBoundedInstallation
...
def CompactParserClosedSuccessTraceBoundedGraph
...
def compactParserClosedSuccessTraceBoundedGraphDef :
  ...
  “tokenTable width tokenCount stateBoundary stateCount fuel
    ...
    !(compactParserInitialFinalBoundedDef)
      ...
      inputBoundary inputCount expectedBoundary expectedCount
      taskKind taskBinderArity taskRepeatCount valueBound ∧
    !(compactParserClosedSuccessAdjacentRowsBoundedGraphDef)
      tokenTable width tokenCount stateBoundary stateCount fuel
      tableWidth valueBound”

@[simp] theorem compactParserClosedSuccessTraceBoundedGraphDef_spec
    (tokenTable width tokenCount stateBoundary stateCount fuel
      inputBoundary inputCount expectedBoundary expectedCount
      taskKind taskBinderArity taskRepeatCount
      tableWidth valueBound : Nat) :
    compactParserClosedSuccessTraceBoundedGraphDef.val.Evalb
        ![tokenTable, width, tokenCount, stateBoundary, stateCount, fuel,
          inputBoundary, inputCount, expectedBoundary, expectedCount,
          taskKind, taskBinderArity, taskRepeatCount, tableWidth, valueBound] ↔
  ...
  let env : Fin 15 → Nat :=
    ![tokenTable, width, tokenCount, stateBoundary, stateCount, fuel,
      inputBoundary, inputCount, expectedBoundary, expectedCount,
      taskKind, taskBinderArity, taskRepeatCount, tableWidth, valueBound]
  change compactParserClosedSuccessTraceBoundedGraphDef.val.Evalb env ↔ _
  ...
  simp [compactParserClosedSuccessTraceBoundedGraphDef,
    CompactParserClosedSuccessTraceBoundedGraph,
    hinitialFinalEnv, hadjacentEnv, hstateCountValue, hfuelValue]

theorem compactParserClosedSuccessTraceBoundedGraphDef_sigmaZero :
    LO.FirstOrder.Arithmetic.Hierarchy LO.Polarity.sigma 0
      compactParserClosedSuccessTraceBoundedGraphDef.val := by
  simp [compactParserClosedSuccessTraceBoundedGraphDef]
...
private theorem parserOutputLocalTraceValid_endpoints
    ...
    (hvalid : CompactParserOutputLocalTraceValid compactClosedSyntaxParserStep fuel
...
theorem CompactParserClosedSuccessTraceBoundedGraph.sound
    {tokenTable width tokenCount stateBoundary stateCount fuel
    ...
    (hgraph : CompactParserClosedSuccessTraceBoundedGraph
      tokenTable width tokenCount stateBoundary stateCount fuel
    ...
    CompactParserOutputLocalTraceValid compactClosedSyntaxParserStep fuel
      (input, [(taskKind, taskBinderArity, taskRepeatCount)], none)
  ...
  have hsteps : ∀ stepIndex < fuel,
      states.getI (stepIndex + 1) =
        compactClosedSyntaxParserStep (states.getI stepIndex) := by
    ...
    rcases hrowGraph.1 with ⟨hcurrentAt, hnextAt, _hordinaryStepRows⟩
    ...
    exact compactUnifiedParserClosedSuccessStepRows_sound
      hcurrentLayout hnextLayout hrowGraph.2
  have hlocal : CompactParserLocalTraceValid compactClosedSyntaxParserStep fuel
...
theorem closedLocalTrace_exists_compactParserClosedSuccessTraceBoundedFormula
  ...
  have hadjacent : CompactUnifiedParserSyntaxStateListAdjacentStepRows
      tokenTable width tokenCount stateBoundary states :=
    syntaxStateListAdjacentStepRows_of_localTrace hrows hsyntax.1 hstartWell
  ...
  have hadjacentBounded : CompactParserClosedSuccessAdjacentRowsBoundedGraph
      tokenTable width tokenCount stateBoundary states.length fuel
        tableWidth (2 ^ tableWidth) := by
    refine ⟨rfl, ?_⟩
...
#print axioms compactParserClosedSuccessTraceBoundedGraphDef_spec
#print axioms compactParserClosedSuccessTraceBoundedGraphDef_sigmaZero
#print axioms CompactParserClosedSuccessTraceBoundedGraph.sound
#print axioms closedLocalTrace_exists_compactParserClosedSuccessTraceBoundedFormula

end FoundationCompactNumericListedDirectParserClosedSuccessTraceFormula
