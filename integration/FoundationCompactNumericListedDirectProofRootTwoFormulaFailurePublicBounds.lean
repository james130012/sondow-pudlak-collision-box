...
/-!
# Public bounds for second-formula proof-root failure

Tags `3` and `4` parse a sequent followed by two formulas.  This module joins
the successful first-formula trace and the complete no-output second-formula
trace in one canonical table, with every exposed coordinate bounded from the
original root-input weight.
-/
...
namespace FoundationCompactNumericListedDirectProofRootTwoFormulaFailurePublicBounds
...
def compactProofRootTwoFormulaFailurePublicCoordinateSizeBound
    (inputWeight : Nat) : Nat :=
  compactProofRootTwoFormulaLocalCoordinateSizeBound inputWeight + 1

structure CompactProofRootTwoFormulaFailurePublicCoordinateBounds
    (tokenTable width tokenCount inputStart inputFinish
...
/-- A second-formula failure constructs its complete bounded endpoint with no
hidden coordinate inputs. -/
theorem
    exists_compactProofRootTwoFormulaFailureEndpointBoundedGraph_of_results_with_publicBounds
    ...
    (hsecond : compactFormulaTokenParser 0 middle = none) :
    let input := tag :: body
    ...
    ∃ bodyStart bodyFinish endpointBound,
      CompactAdditiveNatListDirectLayout
        ...
        CompactProofRootTwoFormulaFailureEndpointBoundedGraph
          tokenTable width tokenCount inputStart inputFinish
            bodyStart bodyFinish endpointBound ∧
        CompactProofRootTwoFormulaFailurePublicCoordinateBounds
          tokenTable width tokenCount inputStart inputFinish
            bodyStart bodyFinish endpointBound
            (compactProofRootTwoFormulaFailurePublicCoordinateSizeBound
              inputWeight) := by
  let input := tag :: body
  ...
  have hsecondOutput : compactSyntaxParserStateOutput
      ((compactSyntaxParserStep^[compactSyntaxRunFuelBound middle])
        (compactFormulaParserInitialState 0 middle)) = none := by
    simpa [compactFormulaTokenParser,
      compactFormulaTokenParserRun] using hsecond
  rcases
      (compactParserOutput_eq_iff_exists_localTrace
        compactSyntaxParserStep (compactSyntaxRunFuelBound middle)
        (compactFormulaParserInitialState 0 middle) none).mp
          hsecondOutput with
  ...
  have hsecondLocal :
    ...
    simpa [compactFormulaParserInitialState,
      compactFormulaTask] using hsecondValid
  rcases
  ...
  have hfirstBounded :
      CompactParserSyntaxExactEndpointBoundedGraph
        tokenTable width tokenCount finalStart finalFinish
          middleStart middleFinish 1 0 0 (2 ^ firstActualBound) := by
  ...
  have hgraph :
    ...
    unfold CompactProofRootTwoFormulaFailureEndpointGraph
    dsimp only [coordinates]
    exact ⟨hinputWitness, hsequentGraph.1,
      hcoordinateTag34, hconsCoordinateTag,
...
end FoundationCompactNumericListedDirectProofRootTwoFormulaFailurePublicBounds
