...
黄色节点的关键链条由 `fvSup`（自由变量上界）、`allClosure`（全称闭包）和 candidate-length guard（候选长度守卫）组成。当前前两项的纯数值本体已经转绿，守卫与完整句合流仍在处理；三者共同面对一个隐蔽的复杂度陷阱。
...
接下来的闭合顺序已经缩窄。纯数值 `fvSup` 已证明精确返回类型化自由变量上界，`allClosure` 本体也已逐 token 对齐依赖类型闭包。下一步是在调用闭包前实现候选位长守卫，复用已经转绿的 `fixitr`，并证明守卫不会改变任何合法接受结果。然后把动态归纳标签 `22` 与已经完成的固定标签 `0..21` 合并，逐条复现 `listedCertificateValidTrace`（列表证书有效性轨迹）。最后，十条证明规则、证明树证书和公式比较共同汇入公开 verifier（验证器），得到完整的原始递归图与逐点结果等式。
