...
解释、层级、正向可靠性和从真实解析／转移出发的反向构造均通过，公理画像仅标准三项。叶分支现只剩
PA（皮亚诺算术）公理叶的联合状态聚合。PA 入口已从真实标签 1 解析恢复闭算术句与 PA 证书；
`Gamma`/候选句/证书三条跨表切片逐位等式也已闭合。证书端点与同一规则分支的联合图现已通过：
fixed/symbol/induction（固定／符号／归纳）三分支均恢复真实 PA 证书并推出公开规则结果，不用只保留
固定公理元数据的统一规则包代替完整证书绑定；当前正接入完整 PA 叶状态。非叶 tag 3--9 的 93 坐标成功聚合也已闭合：
七个规范构造器均在状态框架内推出 `statusTag = 0`（成功状态标签），正向语义与从真实 combine
transition（合并转移）出发的反向构造均通过；任务栈只消费一次，不重复 `drop`。当前随后闭合 PA 叶、
解析器总图和 parse state（解析状态）逐点 `iff`（当且仅当）。
