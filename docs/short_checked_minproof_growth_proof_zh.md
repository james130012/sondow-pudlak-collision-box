...
非叶节点 parse scheduling（解析调度）现已闭合：任意一子节点标签 `4--8` 精确生成
`parse :: combine(root) :: tail`，任意二子节点标签 `3/9` 精确生成
`parse :: parse :: combine(root) :: tail`。任意索引任务行、消费一个旧表头并保留完整尾部、根切片与
combine task（合并任务）相等，均已形成直接 `Sigma-zero`（有界算术）公式；语义、层级和禁用依赖
探针全部通过，公理画像仅标准三项。
...
旧单表失败载荷图已进一步与状态更新行合成 38 坐标失败分支，再嵌入当前态／下一态核心、真实解析任务头，
形成 71 坐标 `Sigma-zero` 失败状态图。该图严格重建两份真实状态布局，并推出
`next = compactNumericVerifierStep current`；同时修正了旧失败状态行把两个相邻列表编码误作一个合并列表
编码的前提。公式解释、层级、语义可靠性和禁用依赖探针均通过，公理画像仅标准三项。该状态层仍需改接
刚闭合的 29 坐标分表 `iff`，并完成规范前后状态到完整失败状态图见证的反向构造。

当前剩余义务是把 29 坐标载荷 `iff` 提升为 parse-state（解析状态）失败分支 `iff`，再把三类叶规则与
公共栈转移汇成一个有界叶分支；随后聚合
非叶成功分支、解析器总图和 parse state（解析状态）逐点 iff（当且仅当）。
